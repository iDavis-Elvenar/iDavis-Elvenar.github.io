function readTomesJSON() {
    ...
    if (searchUsed) {
        ...
        if (location.href.split('#').length > 1 && location.href.split('#')[1] !== "") {
            filteredData = searchForTomeID(location.href.split('#')[1]);
        } else {
            filteredData = data;
        }
    }

    filteredData = filteredData.filter(tome => !tome["id"].startsWith("testing_"));
    ...
    for (var i = 0; i < filteredData.length; i++) {
        ...
        td11.style.width = "60%";
        td11.innerHTML = `<center><img src="${rskIcons[filteredData[i]["iconId"]]}"></center>`;
            ...
            switch (filteredData[i]['rewards'][j]['type']) {
                ...
                case "flexible_reward": {
                    ...
                    if (instants.hasOwnProperty(rewardCodeName)) {
                        ...
                        if (xlImages.hasOwnProperty(flexibleRewards.filter(elem => elem.id === filteredData[i]['rewards'][j]['subType'])[0]['rewards'][getPresetChapter()-1]['subType'])) {
                            td21.innerHTML = `<img src="${xlImages[flexibleRewards.filter(elem => elem.id === filteredData[i]['rewards'][j]['subType'])[0]['rewards'][getPresetChapter()-1]['subType']]}" style="width: 62px;">`;
                        } else {
